...
public class UserLoginAuthenticationProvider implements AuthenticationProvider {
	...
	// 이게 돈줄 유저서비스였는데 스프링 시큐리티 유저디테일서비스로 변경
	UserDetailsService servcie;
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		// 여기가 돈줄 유저 도메인에 있는 유저를 가져오지 않고  
		// 권한을 탈 수 있는 시큐리티 유저로 가져오도록 변경해서 권한을 태워봤습니다.
		UserDetails user = (org.springframework.security.core.userdetails.User)servcie.loadUserByUsername(userId);
		
		/* 인증 진행 */
		
		// DB에 정보가 없는 경우 예외 발생 (아이디/패스워드 잘못됐을 때와 동일한 것이 좋음)
		// ID 및 PW 체크해서 안맞을 경우 (matches를 이용한 암호화 체크를 해야함)
		if (user == null || !userId.equals(user.getUsername()) || !pwEncoding.matches(userPw, user.getPassword())) {
