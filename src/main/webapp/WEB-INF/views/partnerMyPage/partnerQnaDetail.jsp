					...
					<tbody class="tbody">
						<c:if test="${ !empty qList }">
							<c:forEach items="${qList }" var="qna" varStatus="status">
								<tr>
									<td>${status.count }</td>
									<td><a class="table-link-title" href="qaDetail.dz?qnaNo=${qna.qnaNo }">${qna.qnaTitle }${qna.qnaNo }</a></td>
					...
					<tbody class="paging-navi">
						<c:if test="${ !empty qList }">
							<tr align="center" height="20">
								<td colspan="6">
									<!-- 이전 --> <c:url value="allQnaListByPartner.dz" var="before">
									...
									</c:forEach> <!-- 페이징 끝 --> <c:url var="after" value="allQnaListByPartner.dz">
										<c:param name="page" value="${ pi.currentPage+1 }"></c:param>
			...
			success : function(data){
				if(data != null){
					...
					for(var i in data.list){
						...
						var qnaTitle = $("<td>").append("<a class='table-link-title' href='qaDetail.dz?qnaNo="+data.list[i].qnaNo+"'>"+data.list[i].qnaTitle+data.list[i].qnaNo+"</a>");
					...
					if(data.pi.currentPage > 1) { 
						...
						tdNavi.append("<a href='allQnaListByPartner.dz?page="+prev+"'>[이전]</a>&nbsp;");
					}else {
						tdNavi.append("[이전]&nbsp;");
					}
					for(var i = data.pi.startPage; i <= data.pi.endPage; i++) { // for문으로 네비버튼
						if(i == data.pi.currentPage) { // 현재페이지는 빨간색
							...
							tdNavi.append("<a href='allQnaListByPartner.dz?page="+i+"'>"+i+"</a>&nbsp;");
						}
					}//end of for()
					if(data.pi.currentPage < data.pi.maxPage) {//현재페이지가 마지막페이지번호보다 작으면 - 다음버튼 활성화
						var next = Number(data.pi.currentPage)+1;
						tdNavi.append("<a href='allQnaListByPartner.dz?page="+next+"'>[다음]</a>&nbsp;");
					}else {
						tdNavi.append("[다음]&nbsp;");
					}
